...
def inequality(prob, obj):
    ...
    v = prob.states_all_section(1)
    m = prob.states_all_section(2)
    T0 = prob.controls(0, 0)
    T1 = prob.controls(0, 1)
    T2 = prob.controls(0, 2)
    ts0 = prob.time_final(0)
    tf = prob.time_final(-1)
    ...
    result.lower_bound(T0, obj.T_max * 0.98)
    result.lower_bound(T1, 0.0)
    result.lower_bound(T2, 0.0)
    result.lower_bound(tf, 0.1)
    result.lower_bound(ts0, 0.05)
    # upper bounds
    result.upper_bound(m, obj.M0)
    result.upper_bound(T0, obj.T_max)
    result.upper_bound(T1, obj.T_max)
    result.upper_bound(T2, obj.T_max * 0.02)
...
flag_savefig = True
savefig_file = "04_Goddard/05b_2knot_"
...
# mass profile
M_init0 = Guess.linear(prob.time[0], 1.0, 1.0)
M_init1 = Guess.linear(prob.time[1], 1.0, 0.6)
...
T_init1 = Guess.linear(prob.time[1], 3.5, 3.5)
...
prob.solve_pos(obj, display_func, ftol=1e-10)
